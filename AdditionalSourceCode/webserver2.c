...
void web2(int fd, int sleepTime, int totalCount, char* src) {
	FD_ZERO(&read_set);
	FD_SET(fd, &read_set);
	tv.tv_sec = 99920;
	tv.tv_usec = 0;
	...
	while (count < totalCount) {
		...
		if((pid2 = fork()) < 0) {
			...
			if(pid2 != 0) {  /* parent */
				...
				sprintf(buffer,"                                                                    %ld.%06ld", currentTime.tv_sec, currentTime.tv_usec);
				fprintf(stderr, "%s\n", buffer);
...
main(int argc, char **argv) {
	...
	if( argc < 4  || argc > 4 || !strcmp(argv[1], "-?") ) {
		printf("usage failed: ./server <port> <sleep time in microsec/sec> <number of timestamps>\n\n");
		exit(0);
	}
