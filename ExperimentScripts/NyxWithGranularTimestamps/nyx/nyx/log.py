...
class LogEntry(object):
  ...
  **Note:** Tor doesn't include the date in its timestamps so the year
  ...
  def __init__(self, timestamp, type, message):
    ...
    #alex's addition
    self.display_message = '%02i:%02i:%02i|systime:%s [%s] %s' % (entry_time[3], entry_time[4], entry_time[5], time.time() ,self.type, self.message)
    #takes the timestamp of when it was printed, not when tor recieves it. Be aware of potential implications.
