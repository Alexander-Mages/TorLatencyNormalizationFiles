...
open(F, "/users/magesap/TorLatencyNormalizationFiles/ExperimentScripts/tingOnDeter/ting/tor/tor-0.4.7.9/src/app/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/users/magesap/TorLatencyNormalizationFiles/ExperimentScripts/tingOnDeter/ting/tor/tor-0.4.7.9/src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/users/magesap/TorLatencyNormalizationFiles/ExperimentScripts/tingOnDeter/ting/tor/tor-0.4.7.9/doc/man/tor.1.txt") or die;
while (<F>) {
